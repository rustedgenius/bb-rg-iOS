...
- (void)makeRequest:(NSURLRequest*)requestUrl
{
    @try {
        if([[NetworkManager sharedManager] isReachable]){
            ...
            NSURL *url =[NSURL URLWithString:[NSString	stringWithFormat:@"%@/signin",kCommonServerUrl]];
        ...
        else 
        {
            ...
            error = [NSError errorWithDomain:@"Common" code:kNoNetworkErr userInfo:userInfo];
            [userInfo release];
...
- (void)connectionTimeOut
{
    @try {
        ...
        error = [NSError errorWithDomain:@"Common" code:kNoServerResponce userInfo:userInfo];
        [userInfo release];
